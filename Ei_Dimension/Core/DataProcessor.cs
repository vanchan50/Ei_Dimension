	...
	public static class DataProcessor
	{
		...
		public static void BinScatterData(List<ProcessedBead> list, bool fromFile = false)
		{
			...
			foreach (var beadD in list)
			{
				...
				// todo check
				Func<int, int> getValFromArr = (int beadVal) =>
					{
						var res = Array.BinarySearch(HistogramData.Bins, beadVal);
						if (res < 0)
							res = ~res;
						return res;
					};
				int i = getValFromArr((int)bead.fsc);
	 			int j = getValFromArr((int)bead.redssc);				
				int k = getValFromArr((int)bead.greenssc);				
				int o = getValFromArr((int)bead.violetssc);
				int l = getValFromArr((int)bead.reporter);
